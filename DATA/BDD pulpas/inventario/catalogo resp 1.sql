...
DELIMITER $$
CREATE TRIGGER estado_catalogo
AFTER INSERT ON invent_catalogo
FOR EACH ROW
BEGIN
    DECLARE nuevo_estado ENUM('disponible', 'stock bajo', 'agotado');

    IF NEW.stock = 0 THEN
        SET nuevo_estado = 'agotado';
    ELSEIF NEW.stock <= 10 THEN -- Cambia el valor 10 según los criterios de tu negocio
        SET nuevo_estado = 'stock bajo';
    ELSE
