...
BEGIN
    ...
    IF JSON_LENGTH(mano_obra) > 0 THEN
        WHILE i < JSON_LENGTH(mano_obra) DO
            ...
            SET cant_personas = JSON_UNQUOTE(JSON_EXTRACT(mano_obra, CONCAT('$[', i, '].cant_personas')));
            SET precio_hora = JSON_UNQUOTE(JSON_EXTRACT(mano_obra, CONCAT('$[', i, '].precio_hora')));
            SET horas_trabajadas = JSON_UNQUOTE(JSON_EXTRACT(mano_obra, CONCAT('$[', i, '].horas_trabajadas')));

            -- Calcular costo total
            SET costo_total = cant_personas * precio_hora * horas_trabajadas;

            -- Insertar detalle de mano de obra
            INSERT INTO mano_obra (pro_id, cat_id, mo_cant_personas, mo_precio_hora, mo_horas_trabajadas, mo_costo_total)
            VALUES (pro_id, cat_id, cant_personas, precio_hora, horas_trabajadas, costo_total);
